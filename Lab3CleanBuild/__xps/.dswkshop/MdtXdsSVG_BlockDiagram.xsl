...
<!--	
	<use   x="{$BLKD_PRTCHAN_W}"                           y="{$blkd_h + $BLKD2KEY_GAP - 16}"  xlink:href="#BlkDiagram_ExtPortsTable"/> 
-->	
...
<!--		
		<xsl:message>Drawing connection for bridge <xsl:value-of select="$brdgInstance_"/></xsl:message>
-->	
		
		<xsl:variable name="brdg_X_"  select="($inner_X + $BRIDGE_GAP + $BUS_LANE_W + (@POSITION * ($periMOD_W + ($BUS_LANE_W * 2))))"/>	
		<xsl:variable name="brdg_Y_"  select="($sbs_Y   + $PROC2SBS_GAP + (@BUSINDEX * $SBS_H) + ceiling($SBS_H div 2) - ceiling($periMOD_H div 2))"/>
...
<!--		
		="/EDKSYSTEM/BLKDSHAPES/BRIDGESHAPES/MODULE/BUSCONNS[(@ORIENTED = 'WEST')]/BUSCONN">	
-->		
		<xsl:variable name="min_bus_idx_" select="math:min(exsl:node-set($bridgeShapes_)/BRIDGECONN[((@POSITION = $brdgPosition_) and (@ORIENTED = 'WEST'))]/@BUSINDEX)"/>
<!--		
		<xsl:variable name="max_bus_idx_" select="math:max(exsl:node-set($bridgeShapes_)/BRIDGECONN[(@POSITION = $brdgPosition_)]/@BUSINDEX)"/>
-->		
		<xsl:variable name="brdg_Y1_"     select="($sbs_Y   + $PROC2SBS_GAP + ($min_bus_idx_ * $SBS_H) + ceiling($SBS_H div 2) - ceiling($periMOD_H div 2))"/>
<!--		
-->		
		<xsl:variable name="brdg_X_"  select="($inner_X + $BRIDGE_GAP + $BUS_LANE_W + (@POSITION * ($periMOD_W + ($BUS_LANE_W * 2))))"/>	
<!--		
		<xsl:variable name="brdg_Y_"  select="($sbs_Y   + $PROC2SBS_GAP + (@BUSINDEX * $SBS_H) + ceiling($SBS_H div 2) - ceiling($periMOD_H div 2))"/>
		<xsl:variable name="bc_Y_"    select="(($sbs_Y + $PROC2SBS_GAP  + (@BUSINDEX * $SBS_H) + 2) - ceiling($BIFC_H div 2))"/>
-->	
		<xsl:variable name="bc_Y_"    select="($sbs_Y + $PROC2SBS_GAP  + (@BUSINDEX * $SBS_H)) - ceiling($BIFC_H div 4)"/>
		
		<xsl:variable name="bc_X_">
			<xsl:choose>
				<xsl:when test="@ORIENTED='WEST'">
					<xsl:value-of select="($brdg_X_ - $BUS_LANE_W)"/>
<!--					
					<xsl:message>Drawing west connection for bridge <xsl:value-of select="$brdgInstance_"/> at <xsl:value-of select="@POSITION"/> </xsl:message>
-->	
					
				</xsl:when>
				<xsl:when test="@ORIENTED='EAST'">
					<xsl:value-of select="($brdg_X_ + $periMOD_W + $BUS_LANE_W - $BIFC_H)"/>
<!--					
					<xsl:message>Drawing east connection for bridge <xsl:value-of select="$brdgInstance_"/> at <xsl:value-of select="@POSITION"/> </xsl:message>
-->	
				</xsl:when>
		...
		<xsl:variable name="vert_line_y1_" select="$brdg_Y1_ + $MOD_LANE_H + $MOD_LABEL_H + $BIF_GAP + ceiling($BIF_H div 2)"/>	
		
		<xsl:variable name="vert_line_y2_">
			<xsl:choose>
				<xsl:when test="(@ORIENTED = 'EAST')">
					<xsl:value-of select="$bc_Y_"/>
				</xsl:when>
				
				<xsl:otherwise> 
					<xsl:value-of select="($bc_Y_ + $BIFC_H)"/>
				</xsl:otherwise>
		...
		<xsl:variable name="horz_line_x2_">
			<xsl:choose>
				<xsl:when test="(@ORIENTED = 'EAST')">
					<xsl:value-of select="$vert_line_x_ - $BUS_LANE_W"/>
				</xsl:when>
		...
		<line x1="{$vert_line_x_}" 
			  ...
			  x2="{$vert_line_x_}"
			  y2="{$vert_line_y2_}" 
			  style="stroke:{$bus_col_};stroke-width:1"/>
...
<!--		
		<xsl:variable name="vert_line_x" select="$brdge_X_ - $BUS_LANE_W + ceiling($BIFC_W div 2)"/>
-->		
...
<xsl:template name="Draw_BlkDiagram_ComplexModules">
	...
	<xsl:for-each select="/EDKSYSTEM/BLKDSHAPES/CMPLXSHAPES/CMPLXSHAPE[(@IS_PROMOTED and not(@IS_PENALIZED) and not(@CSTACK_INDEX) and @GAPS_X and @MODS_X and @BUS_LANES_X and @BKT_LANES_X and @BKT_GAPS_X)]">
		...
		<xsl:for-each select="BUSCONNS[@ORIENTED = 'WEST']/BUSCONN[@IS_SBSBIF and @BUSLANE_X and @BUSINDEX]">
			...
			<xsl:variable name="westSbsBusY_"  select="($SharedBus_Y    + (@BUSINDEX * $SBS_H))"/>
		...
		<xsl:for-each select="BUSCONNS[(@ORIENTED = 'EAST')]/BUSCONN[@IS_SBSBIF and @BUSLANE_X and @BUSINDEX]">
			...
			<xsl:variable name="eastSbsBusY_"  select="($SharedBus_Y    + (@BUSINDEX * $SBS_H))"/>
...
<xsl:template name="Draw_BlkDiagram_SharedBusBuckets">
	...
	<xsl:for-each select="/EDKSYSTSEM/BLKDSHAPES/SBSBUCKETS/SBSBUCKET[not(@PROCESSOR) and @BUS_LANES_X and @GAPS_X and @MODS_X ]">			
		...
		<xsl:for-each select="BUSCONNS/BUSCONN[(@IS_BKTCONN and @BUSSTD and @BUSLANE_X and @BUSINDEX)]">	
			...
			<xsl:variable name="bktSbsCTop_"  select="($SharedBus_Y + (@BUSINDEX * $SBS_H) - ceiling($BIFC_H div 2) + ($BUS_ARROW_G * 2))"/>
...
<xsl:template name="Draw_BlkDiagram_Proc2ProcConnections">
	...
	<xsl:for-each select="/EDKSYSTEM/BLKDSHAPES/PROCSHAPES/MODULE/BUSCONNS/BUSCONN[(@IS_PROC2PROC = 'TRUE') and (@IS_SPLITCONN = 'TRUE')]">
		...
		<xsl:variable name="proc_h_"          select="(($MOD_LANE_H * 2) + (($BIF_H + $BIF_GAP) * $pbifsH_) + ($MOD_LABEL_H + $BIF_GAP))"/>	
		...
		<xsl:for-each select="BUSCONN">
			<xsl:choose>
				...
				<xsl:when test="(@PBIF_Y and @BIFRANK)">
					...
					<xsl:variable name="mp_proc_bif_dy_" select="((($BIF_H + $BIF_GAP) * @PBIF_Y) + ($MOD_LABEL_H + $BIF_GAP + $MOD_LANE_H))"/>	
...
-->		
		...
		<xsl:variable name="sbus_bc_y_" 			   select="($SharedBus_Y + (@BUSINDEX * $SBS_H))"/>
		<xsl:variable name="mp_stack_y_"			   select="($SharedBus_Y - ($PROC2SBS_GAP + $Max_Proc_H + $Max_Proc_PerisAbvSbs_H + $mp_stack_h_))"/>
