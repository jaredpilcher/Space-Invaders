...
void slaveHandler(){
	...
	print("*");
	slave_buttons = XIo_In32(0x40080000 + 8);
	slave_switches = XIo_In32(0x40080000 + 4);
...
void render(){
		  ...
		  //while(XIo_In32(XPAR_AUDIO_DMA_0_BASEADDR + 16) != 0){};
		  ...
		  if (slave_switches & 0x80){
				XIo_Out32(0x40080000, to_hex_display(aliens_alive));
		  } else if (slave_switches & 0x40){
				XIo_Out32(0x40080000, to_hex_display(lives));
		  } else {
...
int main() {
	...
	XExc_RegisterHandler(XEXC_ID_NON_CRITICAL_INT, non_critical_handler, &ac97_opb);
	enableAudioInterrupt();
	enableSlaveInterrupt();
